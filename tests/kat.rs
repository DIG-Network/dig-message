...
// ── WU2 placeholders (seal + BLS signature, SPEC §5) — implemented in #1160. ──
//
// The KAT vectors below are OWNED BY WU2; the envelope/InnerMessage FIELDS they populate
// (SealedPayload.kem_enc/ciphertext, InnerMessage.sender_sig) are already final in this crate.
//   - seal/open round-trip with fixed keys (test nonces DERIVED from a hashed seed — CodeQL)
//   - relay-sees-only-ciphertext (no plaintext substring on the wire)
//   - tampered-AAD / tampered-sig rejection
//   - wrong-recipient / wrong-sender-pubkey decrypt-fail
//   - non-subgroup / identity-point kem_enc -> REJECT
//   - compressed(id=1) compress->seal->open->decompress == original
//   - the G1-DH round-trip agrees sender-side == recipient-side
//   - BLS sender-sig verify/reject + the domain-separation-vs-chain-AGG_SIG test (SPEC §5.1a)
//   - self-addressed (sender key == recipient key) round-trip (SPEC §5.6a)
